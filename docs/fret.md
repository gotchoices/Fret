...
- **peerStore protocols (when `identify` has run).** On `peer:identify` / `peer:update`, the peer's negotiated-protocol list is checked: contains one of ours → `member`; non-empty but contains none → `foreign`; empty → left `unknown`. The same check also runs over the start-time / per-tick seed poll: `seedFromPeerStore` enumerates `peerStore.all()` and classifies each still-`unknown` entry off its protocol list, so a peer whose `identify` completed before this service started is labelled at `start()` without waiting for an event or an outbound probe. This also delivers re-admission — a peer that later starts serving this network re-identifies and is re-evaluated `foreign → member`.
